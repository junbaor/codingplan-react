...
export function PlanPage({ plan }: { plan: PlanPageData }) {
  ...
  return (
    <div className={accentClass[plan.accent]}>
      <SiteHeader locale="zh-CN" links={[{ label: '快速对比', href: '/#compare' }, { label: '全部套餐', href: '/#platforms' }, { label: '常见问题', href: '#faq' }]} />
      <main>
        {archived && <div className="mt-16 border-b border-brand-orange/30 bg-brand-orange/10 py-3"><div className="page-shell flex items-center justify-center gap-2 text-center text-sm text-brand-orange"><CircleAlert size={17} />该产品已下线，本页为历史归档，不提供购买入口。</div></div>}
        <section className={`relative overflow-hidden pb-16 ${archived ? 'pt-20' : 'pt-32'} sm:pb-24 sm:pt-40`}>
          ...
          <div className="page-shell text-center">
            <a href="/" className="focus-ring mb-7 inline-flex min-h-11 items-center gap-2 text-sm text-ink-soft hover:text-ink"><ArrowLeft size={16} />返回全部对比</a>
      ...
      <SiteFooter disclaimer="价格和额度信息来源于各平台官网，可能随时变化，请以官网结算页为准。本站部分链接为推广链接。">
...
function PlanContentBlock({ plan, block }: { plan: PlanPageData; block: NonNullable<PlanPageData['contentOrder']>[number] }) {
    ...
    <ContentSection title={plan.plansTitle} description={plan.plansDescription} surface>
      <div className={`grid gap-5 ${plan.plans.length > 3 ? 'md:grid-cols-2 xl:grid-cols-4' : 'lg:grid-cols-3'}`}>
        {plan.plans.map((item) => <article key={item.name} className={`plan-detail-card relative flex flex-col rounded-3xl border p-6 ${item.featured ? 'accent-border accent-bg' : 'border-border bg-surface'} ${item.disabled ? 'border-dashed' : ''}`}>
          {item.featured && <span className="accent-button absolute -top-3 left-6 rounded-full px-3 py-1 text-xs font-bold">推荐</span>}
          ...
          <div className="mt-6 rounded-xl border border-border bg-surface-raised p-4"><span className="font-mono text-xs uppercase tracking-wider text-ink-muted">适合人群</span><p className="mt-1 text-sm text-ink-soft">{item.audience}</p></div>
          {plan.purchaseUrl && !item.disabled && <a href={plan.purchaseUrl} target="_blank" rel="noopener noreferrer" className="plan-detail-cta focus-ring accent-button mt-5 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 text-sm font-bold transition">{item.ctaLabel ?? `立即购买 ${item.name} →`}<ArrowUpRight size={16} /></a>}
        </article>)}
      </div>
    </ContentSection>
  )
  ...
  if (block === 'faq') return <section id="faq" className="scroll-mt-20 py-16 sm:py-24"><div className="page-shell"><SectionHeading title="常见问题" /><FaqList faqs={plan.faqs} /></div></section>
