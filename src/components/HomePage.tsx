              ...
              <table className="w-full min-w-[900px] border-collapse text-left text-sm">
                ...
                <tbody>
                  {data.platforms.map((platform) => (
                    <tr key={platform.id} className="border-b border-border last:border-0 hover:bg-surface-hover/60">
                      <td className="px-5 py-4"><a className="focus-ring font-bold hover:text-brand-cyan" href={`#${platform.id}`}>{platform.name}</a></td>
                ...
                <article id={platform.id} key={platform.id} className={`${accentClass[platform.accent]} scroll-mt-24 overflow-hidden rounded-3xl border bg-surface ${platform.availability === 'archived' ? 'border-dashed border-border-strong' : 'border-border'}`}>
                  <div className="accent-border border-l-4 px-5 py-6 sm:px-8">
                    <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
                      <div>{isEn ? <h3 className="text-2xl font-black tracking-tight sm:text-3xl">{platform.heading}</h3> : <h2 className="text-2xl font-black tracking-tight sm:text-3xl">{platform.heading}</h2>}<p className="mt-1 text-sm text-ink-soft sm:text-base">{platform.subtitle}</p></div>
